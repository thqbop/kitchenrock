...
from kitchenrock_api.models import (
    User, UserPref, UserEmail, Api,
)
from kitchenrock_api.const import (
     ResourceType
)
import re
from kitchenrock_api.services.auth import AppKeyService
...
class UserService(BaseService):
    ...
    def get_users(cls, *args, **kwargs):
        ...
        if search:
            ...
            user_ids = User.objects.values_list('id', flat=True).order_by(order_by).filter(**filter).exclude(
                **excludes)[offset:end]
            count = User.objects.values_list('id', flat=True).order_by(order_by).filter(**filter).exclude(
                **excludes).count()
        if id_only:
            return user_ids
        for id in user_ids:
