...
from kitchenrock.settings.base import SUPPORT_EMAIL
...
class EmailService(BaseService):
    ...
    def _sm_send_email(cls, subject, to_add, context, template_name, reply_to=None, queue=True):
        ...
        if queue:
            queued = QueueService.async_job(QueueService.QUEUE_NAME_EMAIL, 'kitchenrock_api.tasks.send_email', data, delayed_time=0)
            return queued
        else:
